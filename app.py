...
def init_db():
    ...
    # Migrasi Kolom
    try: c.execute("ALTER TABLE req_barang ADD COLUMN alasan_tolak TEXT DEFAULT '-'")
    except sqlite3.OperationalError: pass
    try: c.execute("ALTER TABLE req_bbm ADD COLUMN alasan_tolak TEXT DEFAULT '-'")
    except sqlite3.OperationalError: pass
    try: c.execute("ALTER TABLE laporan_bbm ADD COLUMN alasan_tolak TEXT DEFAULT '-'")
    except sqlite3.OperationalError: pass
...
# ==========================================
    ...
    if menu == "Beranda":
        ...
        with st.expander("📋 Lihat Seluruh Persediaan Barang"):
        ...
        @st.cache_data
        def convert_to_excel(df, sheet_name="Data"):
            output = io.BytesIO()
        ...
        with tab1:
            ...
            if not df_b.empty:
                st.download_button("📥 Export Stok ke Excel", convert_to_excel(df_b, "Stok_Barang"), f"Laporan_Stok_{datetime.date.today()}.xlsx")
                
        with tab2:
            df_restok = pd.read_sql_query("SELECT * FROM log_restok", conn)
            st.dataframe(df_restok, use_container_width=True, hide_index=True)
            if not df_restok.empty:
                st.download_button("📥 Export Log Restok ke Excel", convert_to_excel(df_restok, "Log_Restok"), f"Log_Restok_{datetime.date.today()}.xlsx")
        ...
        with tab3:
            ...
            if not df_penggunaan_bbm.empty:
                st.download_button("📥 Export Laporan BBM ke Excel", convert_to_excel(df_penggunaan_bbm, "Laporan_BBM"), f"Laporan_BBM_{datetime.date.today()}.xlsx")